...
function crearTarjetaGrupo(grupo) {
    ...
    metadatos.append(
        ...
        crearPildoraMeta('Subtareas', String(grupo.total_tareas)),
    ...
    metaPie.append(
        crearElemento(
            ...
            grupo.tareas_pendientes > 0
                ? `${grupo.tareas_pendientes} subtareas pendientes`
                : grupo.estado === 'completado'
                    ? 'Todas las subtareas están completas'
    ...
    acciones.append(
        crearBotonAccion('Editar subtareas', 'editar-tareas', grupo.id, 'button--ghost button--small'),
        crearBotonAccion('Eliminar tarea', 'eliminar-grupo', grupo.id, 'button--danger button--small')
    );
...
function crearFormularioEdicionGrupo(grupo) {
    ...
    formulario.append(
        crearCampoTexto({
            ...
            label: 'Editar nombre de la tarea',
            name: 'titulo',
            maxLength: 120,
            value: grupo.titulo,
...
function crearFormularioCrearTarea(idGrupo) {
    ...
    encabezado.append(
        crearElemento('h4', 'inline-form__title', 'Agregar subtarea'),
        crearElemento('p', 'inline-form__description', 'Crea una subtarea dentro de esta tarea.')
    );
    ...
    formulario.append(
        ...
        crearCampoTexto({
            ...
            label: 'Descripción de la subtarea',
            name: 'descripcion',
            maxLength: 2000,
            placeholder: 'Notas, obvservaciones...',
            rows: 2,
            compact: true,
        })
    );
    ...
    const acciones = crearElemento('div', 'inline-form__actions');
    const botonGuardar = crearElemento('button', 'button button--primary button--small', 'Agregar subtarea');
    botonGuardar.type = 'submit';
...
function crearSeccionTareas(grupo, { editable = false } = {}) {
    ...
    encabezado.append(
        crearElemento(
            ...
            editable ? 'Editar subtareas de la tarea' : 'Tareas del grupo'
        ),
        crearElemento(
            'span',
            'tasks-section__caption',
            grupo.total_tareas > 0 ? `${grupo.total_tareas} registradas` : 'Sin subtareas todavía'
        )
    );
    seccion.appendChild(encabezado);
    ...
    if (!Array.isArray(grupo.tareas) || grupo.tareas.length === 0) {
        seccion.appendChild(
            crearElemento(
                'div',
                'task-empty-state',
                'Esta tarea aún no tiene subtareas. Agrega la primera desde el formulario superior.'
            )
        );
        return seccion;
    }
...
function crearFormularioEdicionTarea(tarea) {
    ...
    const acciones = crearElemento('div', 'inline-form__actions');
    const botonGuardar = crearElemento('button', 'button button--primary button--small', 'Guardar subtarea');
    botonGuardar.type = 'submit';
    acciones.append(
        botonGuardar,
...
async function manejarCreacionGrupo(event) {
    ...
    try {
        ...
        formulario.reset();
        mostrarMensaje(elementos.mensajeFormularioGrupo, datos.mensaje || 'Tarea creada correctamente.', 'exito');
...
async function eliminarGrupo(boton) {
    ...
    if (!window.confirm('Se eliminará la tarea completa con todas sus subtareas. ¿Deseas continuar?')) {
        return;
    }
...
async function enviarCreacionTarea(formulario) {
    ...
    try {
        ...
        formulario.reset();
        await cargarGrupos({ mostrarCarga: false });
        mostrarMensaje(elementos.mensajePagina, datos.mensaje || 'Subtarea creada correctamente.', 'exito');
        mostrarMensaje(elementos.mensajeModal, datos.mensaje || 'Subtarea creada correctamente.', 'exito');
    } catch (error) {
        mostrarMensaje(elementos.mensajePagina, error.message, 'error');
        mostrarMensaje(elementos.mensajeModal, error.message, 'error');
    } finally {
...
async function eliminarTarea(boton) {
    ...
    if (!window.confirm('Se eliminará esta subtarea. ¿Deseas continuar?')) {
        return;
    }
...
function crearContenidoModal(grupo) {
    ...
    if (estadoAplicacion.modoModal === 'edicion-grupo' || estadoAplicacion.modoModal === 'edicion-tareas') {
        ...
        acciones.append(
            ...
            crearBotonAccion('Editar subtareas', 'cambiar-a-edicion-tareas', grupo.id, 'button--ghost button--small'),
            crearBotonAccion('Eliminar tarea', 'eliminar-grupo', grupo.id, 'button--danger button--small')
        );
    }
...
function obtenerSubtituloModal() {
    if (estadoAplicacion.modoModal === 'edicion-grupo') {
        return 'Edita el nombre y la descripción de la tarea.';
    }

    if (estadoAplicacion.modoModal === 'edicion-tareas') {
        return 'Agrega, edita o elimina subtareas de esta tarea.';
    }
